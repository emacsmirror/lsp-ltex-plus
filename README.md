...
The two middle rows are what typing produces: one paragraph edited, the rest of the document unchanged. The server keeps a paragraph cache, so only what changed is analysed again — and, with the hosted service, only what changed is sent over the network, which is why the 15 KB document costs barely more than the page. Change nothing and nothing is sent at all: the buffer goes out only when an edit has been waiting. Most of what the 15 KB document costs is not the changed paragraph either — re-sending it with nothing changed still takes 60 of those milliseconds, which is the whole of it going over the wire and being parsed again. That fixed cost is what grows with the size of a document, and what raising the delay spares you.

**The wait is the part you choose.** `lsp-ltex-plus-idle-delay` is how long after your *last* keystroke the buffer is sent, and every edit restarts it: while you type steadily nothing goes out, and a burst of typing is sent once, when it stops. That is deliberate — every send carries the whole document, because the server synchronises in full — but it means the underlines follow your pauses, not your keystrokes. Lower it for quicker feedback; raise it on a slow machine or for very large documents. There is no other cadence to tune: flymake, or flycheck, draws the underlines the moment the answer arrives.

**The check is the small part of it,** locally: with the default delay you see a correction about half a second after you stop typing, and the half second is almost all of it. The hosted service adds a network round trip for whatever changed, which was five times the local check and still under a fifth of a second — but it has a long tail the local backend does not: in twenty re-checks one took several seconds and one took the best part of a minute. Network conditions and how busy the service is decide, so read that column as the shape of it rather than as measurements. The one-off six seconds is the JVM and LanguageTool's language model loading, whichever backend you use (see [Startup Delay When Opening the First Buffer](#startup-delay-when-opening-the-first-buffer)).
